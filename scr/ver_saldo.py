...
def ver_saldo(login, senha, janela):
    ...
    if usuario == None or usuario == "":
        ...
        txt = tk.Message(janela, text= f"Seu saldo é de R${saldo}.")
        txt.grid(row=4, column=1)
...
def logar():
    ...
    tk.Button(janela, text="Logar", command=lambda: ver_saldo(login, senha, janela)).grid(row=3, column=1)
